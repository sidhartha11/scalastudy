...
trait GarbageHold[T] {
  ...
  def reverseGraph: GarbageHold[T]
}
 ...
 * private class GarbageHoldImpl[T]
 * 
 ...
 */
object GarbageHold {
  def apply[T](biDir: GraphConstants.Value = GraphConstants.directed): GarbageHold[T] =
    new GarbageHoldImpl[T](biDir)
   ...
   */
  private class GarbageHoldImpl[T](
    private val biDir: GraphConstants.Value) extends GarbageHold[T] {
     ...
     * </pre>
     *
     * @see org.geo.scala.graph.sedgewick.adjacency.GarbageHold#printGraph()
     */
    def printGraph = {
      numberEdges = 0
      numberVertices = 0
     ...
     * @see org.geo.scala.graph.undirected.GarbageHold#V()
     */
    def V: Int = numberVertices

    /**
     * returns the number of edges
     * @see org.geo.scala.graph.undirected.GarbageHold#E()
     */
    def E: Int = {
      biDir match {
    ...
    override def toString = "GarbageHold:" + graphMap.mkString("[", ",", "]")
    ...
    def reverseGraph: GarbageHold[T] = {
      ...
      val revGraph:GarbageHold[T] = GarbageHold[T]()
      for ( (k,v) <- getGraph ) {
        for ( (k1,v1 ) <- v ) {
